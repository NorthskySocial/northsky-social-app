...
const BLACKSKY_PROFILE: app.bsky.actor.defs.ProfileViewDetailed = {
  did: 'did:plc:profile',
  handle: 'profile.test',
...
const BLUESKY_PROFILE: app.bsky.actor.defs.ProfileViewDetailed = {
  did: 'did:plc:profile',
  handle: 'profile.test',
...
describe('getProfileWithGlobalCounts', () => {
  ...
  it('uses the routed profile when Bluesky fails', async () => {
    ...
    await expect(
      getProfileWithGlobalCounts(BLACKSKY_APPVIEW, 'getProfile', request),
    ).resolves.toBe(BLACKSKY_PROFILE)
  })
